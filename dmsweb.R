...
server <- function(input, output, session) {
  ...
  output$yearControl <- renderUI({
    ...
    sliderInput(inputId = "dates", 
                ...
                value = c(1988, max_year),
                step = 1,
                sep = "")
  })
  
  observe({
    updateSelectInput(session, "country1", choices = cc$country, selected = "Germany")
  ...
  plotInput <- reactive({
    ...
    s1 <- data.frame(demosup[demosup$country==input$country1, 
                         c("country", "year", "mean", "sd")])
    
    if(input$country2 != "select") {
      s2 <- data.frame(demosup[demosup$country==input$country2, 
                           c("country", "year", "mean", "sd")])
      s1 <- merge(s1, s2, all=T)
    }
    
    if(input$country3 != "select") {
      s3 <- data.frame(demosup[demosup$country==input$country3, 
                           c("country", "year", "mean", "sd")])
      s1 <- merge(s1, s3, all=T)
    }
    
    if(input$country4 != "select"){
      s4 <- data.frame(demosup[demosup$country==input$country4, 
                           c("country", "year", "mean", "sd")])
    ...
    s1 <- cbind(s1, s2[, 3])
    names(s1)[5] <- "value_se"
    s1$variable <- gsub("mean", "Public Support for Democracy", s1$variable)
...
ui <- shinyUI(fluidPage( 
  ...
  fluidRow(        
    column(3,
           ...
           selectInput("country1", label="Countries", "Germany"), 
